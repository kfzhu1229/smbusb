...
int readFlash(int address, int len, unsigned char* buf) {
	...
	for (i=0;i<len/CHUNKLEN;i++) {
		...
		//if (status != 2) printf("Status: %d", status); //return -1;
		if (status < 0) return -1;

		status=SMBReadBlock(0x16,CMD_READ_BLOCK,chunk);	
		printf("Current Chunk: %.4s; Current i: %d; Current len: %d\n", chunk, i, len);
...
int writeFlash(int address, int len, unsigned char* buf) {
	...
	for (i=0;i<len/CHUNKLEN;i++) {
		...
		//if (status != CHUNKLEN+2) printf("Note: status = %d != CHUNKLEN+2 = %d\n", status, CHUNKLEN+2); // return status;		
